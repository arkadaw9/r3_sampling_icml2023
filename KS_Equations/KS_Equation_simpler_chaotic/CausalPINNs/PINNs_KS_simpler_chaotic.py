...
parser = argparse.ArgumentParser(description='Causal Training for PINNs: CausalPINNs')
parser.add_argument('--n_x', type=int, default=256, help='')
parser.add_argument('--n_t', type=int, default=32, help='')
parser.add_argument('--experiment_name', type=str, default="experiment_1", help='')
args = parser.parse_args()

print(args)

save_path = f"./results/{args.experiment_name}/n_x_{args.n_x}_n_t_{args.n_t}/"
...
for k in range(N):
    ...
    np.save(os.path.join(save_path,'u_pred_list.npy'), u_pred_list)
    np.save(os.path.join(save_path,'params_list.npy'), params_list)
    np.save(os.path.join(save_path,'losses_list.npy'), losses_list)
