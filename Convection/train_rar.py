...
from utils import *
from pinn_model_rar import PINN
from dataset import DatasetGenerator
from parameters_rar import get_params
from systems import *
import os
...
# Arguments
################
parser = argparse.ArgumentParser(description='RAR-Based Methods for for PINNs')

parser.add_argument('--seed', type=int, default=1234, help='Random initialization.')
parser.add_argument('--method', type=str, default="rar_g", help='Type of method: rar_g, rad, rar_d, l_inf')
...
parser.add_argument('--save_dir', type=str, default="", help='Save path of results.')
...
parser.add_argument('--N_s', type=int, default=10000, help='Number of points in the dense sample set.')
parser.add_argument('--k', type=int, default=1000, help='Resampling Period.')
parser.add_argument('--m', type=int, default=5, help='Number of points to add to the training set.')

#causal_gate_params
parser.add_argument('--beta', type=float, default=0.0, help='Shift parameter for gate')
parser.add_argument('--alpha', type=float, default=10.0, help='Steepness parameter for gate')
parser.add_argument('--gate_type', type=str, default="relu_tanh", help='Gate Function')
parser.add_argument('--beta_lr', type=float, default=1e-4, help='Beta learning rate')
...
#Results dir
sub_dir = f"Seed_{args.seed}_{args.system}_nu{args.pde_nu}_rho{args.pde_rho}_beta{args.pde_beta}_initial_{args.u0_str}/{args.dnn_type}_layers_{args.layers}_activation_{args.activation}/lr_{args.lr}_uselrsch_{args.use_lr_scheduler}_decayrate_{args.decay_rate}_decaysteps_{args.decay_steps}/maxiter_{args.max_iter}_lambda_ic_{args.lambda_ic}_lambda_f_{args.lambda_f}_lambda_bc_{args.lambda_bc}/{args.method}_timewindow_{args.N}/"

if args.method in ["rar_g", "rar_d"]:
    subsub_dir = f"N_f_{args.N_f}_N_s_{args.N_s}_resample_period_{args.k}_m_{args.m}"
elif args.method in ["rad", "l_inf"]:
    subsub_dir = f"N_f_{args.N_f}_N_s_{args.N_s}_resample_period_{args.k}"
args.layers = [int(item) for item in args.layers.split(',')]

if args.save_dir != "":
    base_dir = os.path.join(args.results_dir, args.save_dir)
else:
